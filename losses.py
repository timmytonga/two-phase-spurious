...
class LossComputer:
    """ Taken from gDRO paper and modified to suit this architecture: since we are predicting group"""
    def __init__(self, criterion, is_robust, dataset, alpha=0.2, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01,
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        group_acc, group_count = self.compute_group_avg(((torch.argmax(yhat, 1)//2) == y).float(), group_idx)
    ...
    def compute_group_avg(self, losses, group_idx):
        """
        using the
            losses array (a batchsize x 1 array with the loss for each example)
            group_idx array (a batchsize x 1 array indicating which group each example belongs to)
        return an array of size n_group x 1 with the average loss per group
        """
