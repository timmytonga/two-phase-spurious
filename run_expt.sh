...
python train_jointly.py --log_dir $JOINTLY_LOG_DIR --seed $SEED --n_epochs $N_EPOCHS_J --batch_size $BATCH_SIZE --lr $LRJ --resnet_width $WIDTH

# then we train reweight
python train_classifier --resnet_width $WIDTH --model_path $MODEL_PATH --lr $LRS --sampling_method 'reweight' --n_epochs $N_EPOCHS_RW --log_dir $RW_LOG_DIR --seed $SEED 

# finally train subsample
python train_classifier --resnet_width $WIDTH --model_path $MODEL_PATH --lr $LRS --sampling_method 'subsample' --n_epochs $N_EPOCHS_SS --log_dir $SS_LOG_DIR --seed $SEED 
